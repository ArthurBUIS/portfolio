...
const DATA = {
  ...
  navcards: [
    { icon: "🎓", href: "my-courses-2025-2026.html", cls: "",
  ...
  projects: [
    { slug: "human-motion-analysis", icon: "🏃", year: "2025",
      ...
      highlights: [
        ...
        { fr: "CNN-1D au niveau vidéo (> 90 % sur 4 exercices) après l'échec d'une approche PointNet au niveau frame ; détection des répétitions par oscillation caractéristique et filtre de Savitzky-Golay ; notation par Dynamic Time Warping et comparaison d'angles articulaires.",
          en: "Video-level 1D-CNN (> 90% on 4 exercises) after a frame-level PointNet approach failed; repetition detection via characteristic oscillation and Savitzky-Golay filtering; scoring via Dynamic Time Warping and joint-angle comparison." },
      ...
      body: [
        ...
        { type:"p", text:{ fr:"Projet en binôme (P41, Roman Lendormy, Arthur Buis ; cours CSC_51073_EP).", en:"A pair project (P41, Roman Lendormy, Arthur Buis; CSC_51073_EP course)." } },
        ...
        { type:"p", text:{ fr:"Point de départ : un jeu Kaggle « Gym Workout / Exercise Video », 980 vidéos, 22 exercices, filmées à l'horizontale ou à la verticale, parfois avec du texte incrusté ou des coupes.", en:"Starting point: a Kaggle \"Gym Workout / Exercise Video\" dataset, 980 videos, 22 exercises, filmed horizontally or vertically, sometimes with overlaid text or cuts." } },
        { type:"p", text:{ fr:"Pour simplifier, on se restreint à 4 exercices bien distincts : pompes, squats, leg extension et curl biceps à la barre.", en:"To simplify, we restrict to 4 clearly distinct exercises: push-ups, squats, leg extension and barbell biceps curl." } },
        { type:"gallery", items:[
        ...
        { type:"p", text:{ fr:"Les données sont des points clés de pose extraits avec MediaPipe : un premier CNN cadre le corps et l'oriente, puis localise 33 points anatomiques en pseudo-3D, (x, y) relatifs à la taille de la vidéo, z relatif au bassin dans une unité arbitraire (une seule caméra).", en:"The data are pose landmarks extracted with MediaPipe: a first CNN frames and orients the body, then locates 33 anatomical points in pseudo-3D, (x, y) relative to the video size, z relative to the pelvis in an arbitrary unit (a single camera)." } },
        { type:"p", text:{ fr:"MediaPipe a été retenu pour sa rapidité sur CPU.", en:"MediaPipe was kept for its speed on CPU." } },
        ...
        { type:"p", text:{ fr:"Points clés manquants : confiance de détection trop faible (mis à <code>None</code>), occlusions, sortie du cadre.", en:"Missing landmarks: detection confidence too low (set to <code>None</code>), occlusions, out of frame." } },
        ...
        { type:"p", text:{ fr:"Les coordonnées MediaPipe sont relatives à la vidéo affichée, inexploitables pour une étude fine du mouvement.", en:"MediaPipe coordinates are relative to the displayed video, unusable for a detailed study of movement." } },
        { type:"p", text:{ fr:"On construit un repère invariant par translation et par rotation (d'après Vemulapalli et al., 2014) : origine au centre des hanches ; longueur du torse (centre hanches → centre épaules) fixée à 1 ; axe x = épaule droite → épaule gauche ; z = x ∧ axe du torse ; y = x ∧ z ; les signes des axes sont revérifiés pour que y pointe vers la tête et z vers la personne.", en:"We build a translation- and rotation-invariant frame (after Vemulapalli et al., 2014): origin at the hip centre; torso length (hip centre → shoulder centre) set to 1; x-axis = right shoulder → left shoulder; z = x × torso axis; y = x × z; axis signs re-checked so y points to the head and z toward the person." } },
        { type:"p", text:{ fr:"Les points sont ensuite tournés dans ce repère unifié.", en:"The points are then rotated into this unified frame." } },
        { type:"p", text:{ fr:"Les erreurs de MediaPipe se propagent dans le repère calculé → coordonnées bruitées.", en:"MediaPipe errors propagate into the computed frame → noisy coordinates." } },
        { type:"p", text:{ fr:"On ajoute donc, par frame, des features supplémentaires : angles entre triplets de points (hanche-genou-cheville…) et distances (tête-épaule, hanche-cheville…).", en:"So per-frame extra features are added: angles between landmark triplets (hip-knee-ankle…) and distances (head-shoulder, hip-ankle…)." } },
        ...
        { type:"p", text:{ fr:"Bascule vers une classification au niveau vidéo → bien moins de données → modèles plus simples (MLP, CNN-1D).", en:"Switch to video-level classification → far less data → simpler models (MLP, 1D-CNN)." } },
        { type:"p", text:{ fr:"Chaque vidéo : 10 frames échantillonnées à intervalles réguliers ; les points clés et angles de chaque frame réduits à la dimension 3 par PCA (meilleur compromis) → vecteur d'entrée de dimension 30.", en:"Each video: 10 frames sampled at regular intervals; each frame's landmarks and angles reduced to dimension 3 by PCA (best compromise) → a 30-dim input vector." } },
        { type:"p", text:{ fr:"Un petit MLP (une couche cachée de 100) dépasse 80 % à l'entraînement ; un CNN-1D (deux couches de convolution 10→5→2 frames par max-pooling, puis deux couches denses de 128, 4 classes) atteint plus de 90 % de précision et est conservé.", en:"A small MLP (one hidden layer of 100) exceeds 80% on training; a 1D-CNN (two convolution layers 10→5→2 frames via max-pooling, then two dense layers of 128, 4 classes) reaches over 90% accuracy and is kept." } },
        { type:"gallery", items:[
        ...
        { type:"p", text:{ fr:"On aligne une répétition de référence, exécutée correctement, sur celle de l'utilisateur avec le Dynamic Time Warping (Kapica, 2019) : le DTW aligne deux séries de longueurs différentes en dilatant, compressant ou suspendant le temps, en cherchant le chemin monotone de <code>(0,0)</code> à <code>(N,M)</code> qui minimise <code>Σ ‖s1[i] − s2[j]‖</code>.", en:"A correctly-performed reference repetition is aligned onto the user's with Dynamic Time Warping (Kapica, 2019): DTW aligns two series of different lengths by dilating, compressing or pausing time, finding the monotone path from <code>(0,0)</code> to <code>(N,M)</code> minimising <code>Σ ‖s1[i] − s2[j]‖</code>." } },
        { type:"p", text:{ fr:"On compare les vitesses d'exécution des répétitions (plus robuste que les positions).", en:"The rep execution speeds are compared (more robust than positions)." } },
        { type:"gallery", items:[
        ...
        { type:"p", text:{ fr:"La similarité cosinus des positions de points s'est révélée trop sensible au bruit de MediaPipe (une petite erreur de normalisation décale tout le squelette) ; pondérer les points par importance n'a pas suffi.", en:"Cosine similarity of landmark positions proved too sensitive to MediaPipe noise (a small normalisation error shifts the whole skeleton); weighting landmarks by importance wasn't enough." } },
        { type:"p", text:{ fr:"Solution retenue : la similarité cosinus d'un ensemble d'angles articulaires (coude, genou, pied…), assez robuste pour donner des scores voisins au même mouvement filmé sous deux angles.", en:"The chosen solution: cosine similarity of a set of joint angles (elbow, knee, foot…), robust enough to give close scores for the same movement filmed from two angles." } },
        { type:"p", text:{ fr:"Affinage : angles pondérés par exercice, puis somme des écarts d'angles pondérés au carré dans une exponentielle négative (bonne discrimination bon / mauvais), puis sigmoïde décentrée → score entre 0 et 1.", en:"Refinement: angles weighted per exercise, then the sum of weighted squared angle differences inside a negative exponential (good / poor separation), then an off-centre sigmoid → a 0-1 score." } },

        { type:"h2", text:{ fr:"Résultats et conclusion", en:"Results and conclusion" } },
        { type:"p", text:{ fr:"Un pipeline fonctionnel de classification et de notation.", en:"A working classification-and-scoring pipeline." } },
        ...
        { type:"p", text:{ fr:"Notation : DTW + comparaison d'angles articulaires distingue les mouvements corrects des exécutions incomplètes ou fautives.", en:"Scoring: DTW + joint-angle comparison distinguishes correct movements from incomplete or faulty ones." } },
        { type:"p", text:{ fr:"Limites : tout dépend de la qualité des points MediaPipe (erreurs de détection, parties hors champ, z imprécis en vue unique), qui se propage dans la normalisation et le calcul d'angles, c'est en grande partie ce qui a fait échouer PointNet.", en:"Limitations: everything depends on MediaPipe landmark quality (detection errors, out-of-frame parts, imprecise z from a single view), which propagates into normalisation and angle computation, largely what made PointNet fail." } },
        { type:"p", text:{ fr:"Peu d'exercices couverts ; les mouvements similaires ou statiques restent difficiles à distinguer.", en:"Few exercises covered; similar or static movements stay hard to tell apart." } },
        { type:"p", text:{ fr:"La notation repose sur de nombreux paramètres réglés à la main par exercice, ce qui limite la généricité.", en:"Scoring relies on many hand-tuned per-exercise parameters, which limits genericity." } },
        { type:"p", text:{
    ...
    { slug: "social-media-classification", icon: "💬", year: "2025",
      ...
      body: [
        ...
        { type:"p", text:{ fr:"Data challenge Kaggle : prédire si un compte X est un influenceur (1) ou un observateur (0), à partir de 154 914 tweets d'entraînement au format JSON (192 colonnes) et de 103 380 tweets de validation non étiquetés.", en:"A Kaggle data challenge: predict whether an X account is an influencer (1) or an observer (0), from 154,914 training tweets in JSON (192 columns) and 103,380 unlabelled validation tweets." } },
        ...
        { type:"p", text:{ fr:"Projet en binôme (MP3,5, Roman Lendormy, Arthur Buis).", en:"A pair project (MP3,5, Roman Lendormy, Arthur Buis)." } },

        { type:"h2", text:{ fr:"Ingénierie des variables", en:"Feature engineering" } },
        { type:"p", text:{ fr:"Nettoyage : suppression de 48 colonnes constantes ; fusion de chaque champ principal avec son équivalent dans <code>extended_tweet</code> (par ex. <code>full_text</code>).", en:"Cleaning: dropped 48 constant columns; merged each primary field with its <code>extended_tweet</code> counterpart (e.g. <code>full_text</code>)." } },
        ...
        { type:"p", text:{ fr:"On retient <code>user.favourites_count</code>, <code>user.listed_count</code> et <code>user.statuses_count</code>, chacun présente deux distributions selon le label, sur des plages très différentes, d'où l'usage de leur logarithme.", en:"We keep <code>user.favourites_count</code>, <code>user.listed_count</code> and <code>user.statuses_count</code>, each shows two label-dependent distributions over very different ranges, hence taking their logarithm." } },
        { type:"gallery", items:[
        ...
        { type:"p", text:{ fr:"<code>quoted_status</code> s'est révélé peu corrélé, mais on garde <code>quoted_status.favorite_count</code> et <code>quoted_status.user.favorites_count</code> (en log).", en:"<code>quoted_status</code> turned out weakly correlated, but we keep <code>quoted_status.favorite_count</code> and <code>quoted_status.user.favorites_count</code> (log)." } },
        { type:"p", text:{ fr:"Le champ <code>source</code> → one-hot partiel : encodage complet des clients dominants (iPhone, Android, iPad, TweetDeck, Hootsuite), puis <code>other_1</code> / <code>other_0</code> pour regrouper les sources mineures selon leur label majoritaire. <code>entities.media</code> → <code>has_photo</code> / <code>has_video</code> / <code>has_gif</code> ; <code>entities.hashtags</code> → one-hot des 20 hashtags les plus fréquents.", en:"The <code>source</code> field → partial one-hot: full encoding of the dominant clients (iPhone, Android, iPad, TweetDeck, Hootsuite), then <code>other_1</code> / <code>other_0</code> to bucket minor sources by their majority label. <code>entities.media</code> → <code>has_photo</code> / <code>has_video</code> / <code>has_gif</code>; <code>entities.hashtags</code> → one-hot of the 20 most frequent hashtags." } },
        { type:"gallery", items:[
        ...
        { type:"p", text:{ fr:"<code>user.created_at</code> (précision à la milliseconde) fait une clé quasi unique par utilisateur, moins de 1 % de collisions dans le train, aucune avec le label.", en:"<code>user.created_at</code> (millisecond precision) makes a near-unique per-user key, under 1% collisions in the training set, none with the label." } },
        { type:"p", text:{ fr:"Une tentative de récupérer le nombre de followers via <code>quoted_status.user.created_at</code> ne couvre qu'environ 1 % du jeu (non utilisée directement).", en:"An attempt to recover follower counts via <code>quoted_status.user.created_at</code> only covers about 1% of the data (not used directly)." } },
        { type:"p", text:{ fr:"Mais regrouper les tweets par <code>user.created_at</code> identifie les tweets d'un même utilisateur : moyenner leurs prédictions en sortie de modèle gagne +0,05 % avec BERTweet.", en:"But grouping tweets by <code>user.created_at</code> identifies a single user's tweets: averaging their predictions at model output gains +0.05% with BERTweet." } },
        { type:"h3", text:{ fr:"Ce que les encodeurs de texte capturent vraiment", en:"What the text encoders actually capture" } },
        { type:"p", text:{ fr:"PCA et t-SNE sur les embeddings CLS d'un BERTweet affiné, séparément pour <code>user.description</code> et <code>full_text</code>.", en:"PCA and t-SNE on the CLS embeddings of a fine-tuned BERTweet, separately for <code>user.description</code> and <code>full_text</code>." } },
        ...
        { type:"p", text:{ fr:"v1, couche d'embedding + positional encoding + couches encodeur de PyTorch, tokenisation manuelle (mot ou emoji, seuls les plus fréquents).", en:"v1, embedding layer + positional encoding + PyTorch encoder layers, manual tokenisation (word or emoji, only the most frequent)." } },
        { type:"p", text:{ fr:"Surapprend au-delà de 2 epochs : 150 000 tweets ne suffisent pas à un vocabulaire représentatif.", en:"Overfits past 2 epochs: 150,000 tweets aren't enough for a representative vocabulary." } },
        ...
        { type:"p", text:{ fr:"v2, encodeur pré-entraîné. Après BERT et BERTweet, CamemBERT (adapté au français) l'emporte.", en:"v2, pre-trained encoder. After BERT and BERTweet, CamemBERT (better for French) wins." } },
        { type:"p", text:{ fr:"Architecture : encodeur de texte → embeddings ; sous-réseau (2 couches linéaires + dropout) sur les variables numériques ; concaténation ; classifieur à 3 couches.", en:"Architecture: text encoder → embeddings; a subnet (2 linear layers + dropout) on the numeric features; concatenation; a 3-layer classifier." } },
        ...
        { type:"p", text:{ fr:"v3, DualBERT : deux encodeurs CamemBERT séparés pour <code>user.description</code> et <code>full_text</code>, sous-réseau métadonnées, tête à 3 couches.", en:"v3, DualBERT: two separate CamemBERT encoders for <code>user.description</code> and <code>full_text</code>, a metadata subnet, a 3-layer head." } },
        { type:"p", text:{ fr:"222 M de paramètres, entraînement en trois phases.", en:"222M parameters, three-phase training." } },
        { type:"p", text:{ fr:"Jusqu'à 83,9 % sur Kaggle, mais ~3 h d'entraînement et aucun gain réel → on garde la v2 (description seule) pour la soumission finale.", en:"Up to 83.9% on Kaggle, but ~3 h of training and no real gain → we keep v2 (description only) for the final submission." } },

        { type:"h2", text:{ fr:"XGBoost", en:"XGBoost" } },
        { type:"p", text:{ fr:"En parallèle, un modèle de boosting d'arbres, bien adapté au mélange de variables numériques, catégorielles et textuelles.", en:"In parallel, a tree-boosting model, well suited to the mix of numeric, categorical and textual features." } },
        { type:"p", text:{ fr:"Prise en main sur une version réduite du jeu (texte, date de création du compte, description) avec TF-IDF + grid search → 0,785.", en:"Warm-up on a stripped-down dataset (text, account creation date, description) with TF-IDF + grid search → 0.785." } },
        { type:"p", text:{ fr:"Jeu complet sans texte → ~0,83 ; ajouter le texte via BERT / BERTweet n'aide pas (modèles anglais), CamemBERT (768-d pour le texte et la description) gagne quelques millièmes.", en:"Full feature set without text → ~0.83; adding text via BERT / BERTweet doesn't help (English models), CamemBERT (768-d for tweet text and description) gains a few thousandths." } },
        { type:"p", text:{ fr:"Même surapprentissage par fuite d'utilisateurs ; le découpage sans utilisateur commun ne relève pas le score Kaggle.", en:"Same user-leak overfitting; the user-disjoint split doesn't lift the Kaggle score." } },
        { type:"p", text:{ fr:"Validation croisée à 5 plis : 0,83-0,84 par pli, score global ≤ 0,83.", en:"5-fold cross-validation: 0.83-0.84 per fold, overall ≤ 0.83." } },
    ...
    { slug: "similarity-graphs", icon: "🕸️", year: "2025",
      ...
      highlights: [
        ...
        { fr: "Pipelines de collecte et de stockage (MongoDB) réunissant 477 artistes et 7 822 collaborations.",
          en: "Collection and storage pipelines (MongoDB) covering 477 artists and 7,822 collaborations." },
      ...
      body: [
        ...
        { type:"p", text:{ fr:"Projet en binôme (modal CSC_43M02_EP, <a href=\"https://www.polytechnique.edu/en\" target=\"_blank\" rel=\"noopener\">École Polytechnique</a>).", en:"A pair project (CSC_43M02_EP research module, <a href=\"https://www.polytechnique.edu/en\" target=\"_blank\" rel=\"noopener\">École Polytechnique</a>)." } },
        { type:"p", text:{ fr:"L'objectif : mettre en graphe les collaborations du rap francophone, en extraire des structures pertinentes, communautés, nœuds à forte centralité, puis analyser le contenu lexical des textes via des embeddings NLP.", en:"The goal: turn French-speaking rap collaborations into graphs, extract meaningful structure, communities, high-centrality nodes, then analyse the lyrics through NLP embeddings." } },
        { type:"p", text:{ fr:"Code : <a href=\"https://github.com/bpmarthur/Modal_Project\" target=\"_blank\" rel=\"noopener\">github.com/bpmarthur/Modal_Project</a>.", en:"Code: <a href=\"https://github.com/bpmarthur/Modal_Project\" target=\"_blank\" rel=\"noopener\">github.com/bpmarthur/Modal_Project</a>." } },
        ...
        { type:"p", text:{ fr:"Chaque artiste porte des métadonnées hétérogènes et un embedding de paroles à 300 dimensions.", en:"Each artist carries heterogeneous metadata plus a 300-dimensional lyrics embedding." } },
        ...
        { type:"p", text:{ fr:"Un premier ensemble d'environ 300 artistes a été constitué par recherche de genre sur Spotify, puis étendu via MusicBrainz (jusqu'à 800 résultats, mais très bruités).", en:"An initial set of about 300 artists was built by genre search on Spotify, then extended via MusicBrainz (up to 800 results, but very noisy)." } },
        { type:"p", text:{ fr:"Un filtrage, identifiants Spotify et Genius présents, minimum de followers, au moins une collaboration connue, a fait converger la base vers 477 artistes.", en:"Filtering, Spotify and Genius IDs present, a minimum follower count, at least one known collaboration, converged the base to 477 artists." } },
        { type:"p", text:{ fr:"L'appariement des identifiants Genius s'est révélé difficile : l'algorithme de recherche privilégie les artistes populaires proches de la requête et bute sur les apostrophes (Rim'K, Sexion d'Assaut).", en:"Matching Genius identifiers proved hard: the search favours popular artists close to the query and trips on apostrophes (Rim'K, Sexion d'Assaut)." } },
        { type:"p", text:{ fr:"Un petit outil de désambiguïsation manuelle a été écrit pour ces cas.", en:"A small manual disambiguation tool was written for these cases." } },
        { type:"p", text:{
        ...
        { type:"p", text:{ fr:"L'idée : relier deux artistes si la similarité cosinus de leurs embeddings dépasse un seuil, avec un poids d'arête proportionnel à cette similarité.", en:"The idea: link two artists when the cosine similarity of their embeddings exceeds a threshold, with edge weight proportional to that similarity." } },
        ...
        { type:"p", text:{ fr:"Un nœud par artiste, une arête pondérée par le nombre de collaborations.", en:"One node per artist, an edge weighted by the number of collaborations." } },
        { type:"p", text:{ fr:"Construction et nettoyage avec NetworkX, visualisation avec Gephi.", en:"Built and cleaned with NetworkX, visualised with Gephi." } },
        ...
        { type:"p", text:{ fr:"Pour comparer avec des données d'écoute réelles : pour chaque artiste, ses 20 plus proches selon le score Last.fm, arête ajoutée si l'artiste similaire est dans la base ; composantes de taille < 5 retirées.", en:"To compare against real listening data: for each artist, its 20 nearest by Last.fm score, an edge added when the similar artist is in the base; components of size < 5 removed." } },
        { type:"p", text:{ fr:"On retrouve des versions élargies des clusters « urbain » et « conscient », un cluster proche du « old school », mais la scène marseillaise disparaît, elle se lit dans les collaborations, pas dans les goûts d'écoute.", en:"Enlarged versions of the \"urban\" and \"conscious\" clusters appear, plus one close to \"old school\", but the Marseille scene vanishes, it shows up in collaborations, not in listening taste." } },
        ...
        { type:"p", text:{ fr:"Une arête si la similarité cosinus dépasse un seuil, avec un poids <code>(similarité − seuil) / (1 − seuil)</code>.", en:"An edge when cosine similarity exceeds a threshold, with weight <code>(similarity − threshold) / (1 − threshold)</code>." } },
        { type:"p", text:{ fr:"Problème : la majorité des similarités dépassent 0,9, difficile de discriminer les artistes.", en:"Problem: most similarities exceed 0.9, artists are hard to tell apart." } },
        { type:"p", text:{ fr:"Le seuil a été monté à 0,98 pour obtenir un graphe lisible ; après retrait des composantes < 5, il ne reste que 275 nœuds (~40 % perdus).", en:"The threshold was raised to 0.98 for a readable graph; after removing components < 5, only 275 nodes remain (~40% lost)." } },
        { type:"gallery", items:[
        ...
        { type:"p", text:{ fr:"Une poignée d'artistes tombent dans un cluster qui ne leur correspond pas tout à fait (Bigflo & Oli, Luidji, Krisy) : casser les codes fait partie des valeurs du rap, ce qui se traduit par un graphe très connecté et des frontières floues.", en:"A handful of artists land in a cluster that doesn't quite fit them (Bigflo & Oli, Luidji, Krisy): breaking genre codes is part of rap's value system, which shows up as a very connected graph with blurred borders." } },
        { type:"h3", text:{ fr:"L'évolution du rap dans le temps", en:"How rap evolved over time" } },
        { type:"p", text:{ fr:"En dimensionnant les nœuds par le degré plutôt que par la centralité, deux dynamiques apparaissent pour les clusters « old school » et « mainstream ».", en:"Sizing nodes by degree rather than centrality reveals two dynamics for the \"old school\" and \"mainstream\" clusters." } },
        ...
        { type:"p", text:{ fr:"À droite (Rim'K, Oxmo Puccino, Orelsan), les artistes ont beaucoup collaboré avec la scène émergente.", en:"On the right (Rim'K, Oxmo Puccino, Orelsan), artists collaborated heavily with the emerging scene." } },
        { type:"p", text:{ fr:"Le degré est fortement corrélé à la centralité (Alonzo, Jul, Leto).", en:"Degree correlates strongly with centrality (Alonzo, Jul, Leto)." } },
        { type:"p", text:{ fr:"Mais Booba détient la plus forte betweenness de son cluster avec bien moins de featurings que Leto : occuper une position de pont ne demande pas forcément beaucoup de collaborations.", en:"But Booba holds the highest betweenness in his cluster with far fewer features than Leto: holding a bridge position doesn't necessarily require many collaborations." } },
        ...
        { type:"p", text:{ fr:"Certains artistes apparaissent comme plusieurs nœuds : ils appartiennent à des groupes classés dans d'autres clusters.", en:"Some artists appear as several nodes: they belong to groups placed in other clusters." } },
        { type:"p", text:{ fr:"Orelsan et Gringe sont en « old school », mais leur duo Casseurs Flowteurs est en « rap conscient » ; Booba est en « mainstream », son duo avec Ali en « old school ».", en:"Orelsan and Gringe sit in \"old school\", but their duo Casseurs Flowteurs is in \"conscious rap\"; Booba is in \"mainstream\", his duo with Ali in \"old school\"." } },
        { type:"h3", text:{ fr:"Last.fm et embeddings, mis en regard", en:"Last.fm and embeddings, compared" } },
        { type:"p", text:{ fr:"Le graphe Last.fm brouille la frontière urbain / conscient, les auditeurs ne se limitent pas à un sous-genre, et efface la scène marseillaise.", en:"The Last.fm graph blurs the urban / conscious border, listeners don't stick to one sub-genre, and erases the Marseille scene." } },
        { type:"p", text:{ fr:"Ses nœuds les plus centraux sont les artistes à cheval sur plusieurs générations (Booba, Orelsan, IAM).", en:"Its most central nodes are the artists straddling several generations (Booba, Orelsan, IAM)." } },
        { type:"p", text:{ fr:"Le graphe des embeddings, lui, déçoit : quelques clusters rappellent des sous-genres mais restent incomplets et hétérogènes.", en:"The embeddings graph disappoints: a few clusters echo sub-genres but stay incomplete and heterogeneous." } },
        { type:"p", text:{ fr:"Deux limites : l'embedding ne repose que sur 5 morceaux par artiste, et moyenner des vecteurs de mots détruit de l'information.", en:"Two limits: the embedding rests on only 5 tracks per artist, and averaging word vectors destroys information." } },
        { type:"h3", text:{ fr:"Popularité et followers Spotify", en:"Spotify popularity and followers" } },
        { type:"p", text:{ fr:"Les valeurs brutes de followers et de popularité sont trop proches d'un artiste à l'autre pour dimensionner les nœuds.", en:"Raw follower and popularity values are too similar between artists to size the nodes." } },
        { type:"p", text:{ fr:"Une courbe de spline les étale et rend l'affichage lisible.", en:"A spline curve spreads them out and makes the display legible." } },
        { type:"p", text:{ fr:"On retrouve alors grossièrement la dynamique degré / centralité, mais avec un poids moindre pour les rappeurs plus anciens (clusters vert, jaune, orange), les jeunes générations streament davantage, tandis que le public du rap ancien écoute souvent sur support physique.", en:"The degree / centrality dynamic then roughly returns, but with less weight on older rappers (green, yellow, orange clusters), younger generations stream more, while the audience for older rap often listens on physical media." } },
        { type:"gallery", items:[
    ...
    { slug: "llm-code-generation", icon: "🤖", year: "2024-2025",
      ...
      highlights: [
        { fr: "Projet scientifique collectif encadré par Lokad : un système fondé sur un LLM qui génère et corrige du code dans Envision, un DSL quasi absent des données d'entraînement des modèles.",
          en: "Lokad-tutored team research project: an LLM-based system that generates and fixes code in Envision, a DSL almost absent from model training data." },
      ...
      body: [
        ...
        { type:"p", text:{ fr:"Objectif du PSC (projet scientifique collectif, 5 étudiants, encadré par Lokad) : un système fondé sur un LLM capable de générer et corriger du code Envision à partir de requêtes en langage naturel.", en:"Goal of the PSC (collective research project, 5 students, tutored by Lokad): an LLM-based system that generates and fixes Envision code from natural-language requests." } },
        ...
        { type:"p", text:{ fr:"Code open-source : <a href=\"https://github.com/SaturnTsen/LLM-DSL-Gen\" target=\"_blank\" rel=\"noopener\">LLM-DSL-Gen</a> et <a href=\"https://github.com/VIU-L/LLM-as-judge-for-DSL\" target=\"_blank\" rel=\"noopener\">LLM-as-judge-for-DSL</a>.", en:"Open-source code: <a href=\"https://github.com/SaturnTsen/LLM-DSL-Gen\" target=\"_blank\" rel=\"noopener\">LLM-DSL-Gen</a> and <a href=\"https://github.com/VIU-L/LLM-as-judge-for-DSL\" target=\"_blank\" rel=\"noopener\">LLM-as-judge-for-DSL</a>." } },
        ...
        { type:"p", text:{ fr:"Volontairement minimal : le but est de fixer une référence de performance avant d'ajouter de l'information ciblée.", en:"Deliberately minimal: the point is to set a performance reference before adding any targeted information." } },
        ...
        { type:"p", text:{ fr:"Un benchmark de 15 problèmes rédigés à la main en anglais, chacun avec une solution Envision détaillée.", en:"A benchmark of 15 hand-written English problems, each with a detailed Envision solution." } },
        { type:"p", text:{ fr:"L'évaluation est automatisée par un second LLM « juge » qui reçoit le code du codeur, le challenge et la solution de référence, explique son raisonnement point par point, puis rend une note binaire 0 / 1.", en:"Evaluation is automated by a second \"judge\" LLM that receives the coder's code, the challenge and the reference solution, explains its reasoning point by point, then returns a binary 0 / 1." } },
        ...
        { type:"p", text:{ fr:"Les erreurs récurrentes ont été corrigées en ajoutant des consignes à son prompt de comportement ; l'incertitude résiduelle sur le benchmark de 15 challenges était d'environ ± 1.", en:"Recurrent errors were fixed by adding instructions to its behaviour prompt; residual uncertainty on the 15-challenge benchmark was about ± 1." } },
        ...
        { type:"p", text:{ fr:"GPT-4o-mini fait moins bien (7,5 / 15), il ajoute des paramètres et des commentaires optionnels qui s'éloignent de la référence et sont pénalisés à tort par le juge.", en:"GPT-4o-mini does worse (7.5 / 15), it adds optional parameters and comments that drift from the reference and get wrongly penalised by the judge." } },
        { type:"p", text:{ fr:"La vraie limite n'est pas la compréhension de la documentation, mais le manque d'information qu'elle contient, ce qui motive la RAG.", en:"The real limitation isn't understanding the documentation but the lack of information in it, which motivates RAG." } },

        { type:"h2", text:{ fr:"Génération augmentée par récupération (RAG)", en:"Retrieval-Augmented Generation (RAG)" } },
        { type:"p", text:{ fr:"Pour chaque requête, récupérer les extraits de documentation pertinents et les injecter dans le prompt.", en:"For each query, retrieve the relevant documentation chunks and inject them into the prompt." } },
        ...
        { type:"p", text:{ fr:"Le découpage naïf en morceaux de longueur égale coupait des exemples de code en deux ; on est passé à un découpage par (sous-)section, avec une pénalité sur les morceaux trop longs.", en:"Naive equal-length chunking split code examples in two; we moved to chunking by (sub)section, with a penalty on over-long chunks." } },
        { type:"p", text:{ fr:"Embeddings via <code>sentence-transformers/multi-qa-mpnet-base-dot-v1</code> ; on retient les 5 meilleurs morceaux de grammaire et les 5 meilleurs de documentation rédigée.", en:"Embeddings via <code>sentence-transformers/multi-qa-mpnet-base-dot-v1</code>; the top 5 grammar chunks and top 5 written-documentation chunks are kept." } },
        ...
        { type:"p", text:{ fr:"Résultat : 8-9 codes compilables, 7-8 jugés corrects sur 15, aucune amélioration, parfois une régression.", en:"Result: 8-9 compilable, 7-8 judged correct out of 15, no improvement, sometimes a regression." } },
        { type:"p", text:{ fr:"Les morceaux extraits sont souvent hors sujet : construire un espace d'embeddings pertinent est difficile pour des textes qui mêlent anglais et code Envision.", en:"Retrieved chunks are often off-topic: building a relevant embedding space is hard for text that mixes English and Envision code." } },
        { type:"h3", text:{ fr:"RAG hybride, le « RAG demander »", en:"Hybrid RAG, the \"RAG demander\"" } },
        { type:"p", text:{ fr:"On ajoute un deuxième LLM qui lit la question et la liste de références, et propose des chapitres de grammaire et des noms de fonctions.", en:"A second LLM reads the question and the reference list and proposes grammar chapters and function names." } },
        ...
        { type:"p", text:{ fr:"Vraie amélioration : 11 compilables, 10 acceptés sur 15.", en:"A real improvement: 11 compilable, 10 accepted out of 15." } },
        { type:"p", text:{ fr:"Exemple : la fonction <code>argmax</code>, nécessaire à un challenge dont l'énoncé ne mentionne jamais « max », une simple recherche par similarité d'embeddings l'aurait manquée, le demander la propose depuis sa connaissance générale.", en:"Example: the <code>argmax</code> function, needed for a challenge whose text never mentions \"max\", plain embedding-similarity retrieval would have missed it, the demander proposes it from general knowledge." } },
        ...
        { type:"table",
          ...
          rows:[
            ...
            [ { fr:"Ajout d'« Envision brief » au prompt du codeur", en:"Adding \"Envision brief\" to the coder prompt" }, "n/a", { fr:"en baisse", en:"drops" },
            ...
            [ { fr:"20 documents RAG au lieu de 10", en:"20 RAG documents instead of 10" }, "n/a", "45",
              { fr:"Du contexte supplémentaire, s'il est bien choisi, ne nuit pas.", en:"Extra context, if well chosen, doesn't hurt." } ],
            [ { fr:"Réordonnancement dynamique des documents à l'échec", en:"Dynamic reordering of documents on failure" }, "n/a", "+1-2",
        ...
        { type:"p", text:{ fr:"Le codeur réussit mieux sur les mots-clés proches de SQL (le <code>IF</code>) que sur ceux propres à Envision : toutes les stratégies de RAG échouent sur les challenges difficiles du <code>WHERE</code>.", en:"The coder does better on SQL-like keywords (<code>IF</code>) than on Envision-specific ones: every RAG strategy fails the hard <code>WHERE</code> challenges." } },
        { type:"p", text:{ fr:"Fort sur l'affichage et les opérations logiques, correct sur le regroupement et le tri, plus faible sur l'interaction avec les fichiers et le filtrage de table, une lacune réelle, filtrer étant central pour un langage d'analyse de données.", en:"Strong on display and logical operations, decent on grouping and sorting, weaker on file interaction and table filtering, a real gap, since filtering is central to a data-analysis language." } },
        { type:"p", text:{ fr:"Principale piste : un fine-tuning ciblé sur ces points faibles.", en:"Main lead: targeted fine-tuning on those weak spots." } },
    ...
    { slug: "urban-trail-optimisation", icon: "🗺️", year: "2022-2023",
      ...
      highlights: [
        ...
        { fr: "Preuve de l'absence de cycle absorbant dans le graphe repondéré, et optimisations (composantes connexes, couronne d'exclusion, passage à A*) pour rendre le calcul praticable.",
          en: "A proof that the reweighted graph has no negative cycle, and optimisations (connected components, exclusion crown, switch to A*) to make the computation tractable." },
      ...
      body: [
        ...
        { type:"p", text:{ fr:"En m'installant en ville, j'ai perdu la diversité de parcours à laquelle le trail m'avait habitué.", en:"When I moved to a city I lost the route variety that trail running had accustomed me to." } },
        { type:"p", text:{ fr:"Aucune application ne génère de <em>nouveaux</em> itinéraires en boucle à partir d'une carte, pour une demande de distance et de dénivelé : Strava propose des parcours déjà empruntés, OpenRunner se construit à la main.", en:"No app generates <em>new</em> looped routes from a map for a target distance and elevation gain: Strava suggests already-run routes, OpenRunner is built by hand." } },
        { type:"p", text:{ fr:"TIPE de classe préparatoire, présenté à l'oral des concours (ENS).", en:"A research project (TIPE) from the preparatory classes for the French Engineering Schools, presented at the competitive oral exams (ENS)." } },
        { type:"p", text:{
        ...
        { type:"p", text:{ fr:"La recherche de cycle de longueur fixée est NP-difficile → une heuristique s'impose.", en:"Fixed-length cycle search is NP-hard → a heuristic is needed." } },
        { type:"p", text:{ fr:"Lewis & Corcoran (2022) construisent des cycles simples avec l'<strong>heuristique à double chemin</strong> de Bhandari : une étape de plus courts chemins disjoints, puis une étape de « détricotage ».", en:"Lewis & Corcoran (2022) build simple cycles with Bhandari's <strong>double-path heuristic</strong>: a disjoint-shortest-paths step, then an \"unknitting\" step." } },
        ...
        { type:"p", text:{ fr:"N'ayant pas trouvé de preuve que le graphe repondéré n'en contient pas, je l'ai démontré (en annexe du rapport).", en:"Having found no proof that the reweighted graph has none, I proved it (in the report's annex)." } },
        ...
        { type:"p", text:{ fr:"Un seul Dijkstra depuis <code>s</code> ; puis pour chaque sommet <code>t ≠ s</code> jugé intéressant, une exécution de Bhandari donne une boucle passant par <code>s</code> et <code>t</code>, dont la (distance, dénivelé) est confrontée à un front de Pareto tenu à jour.", en:"One Dijkstra from <code>s</code>; then for every vertex <code>t ≠ s</code> deemed interesting, one Bhandari run yields a loop through <code>s</code> and <code>t</code>, whose (distance, elevation) is checked against a running Pareto front." } },
        { type:"p", text:{ fr:"En sortie : un ensemble de boucles Pareto-optimales et leurs caractéristiques.", en:"Output: a set of Pareto-optimal loops and their characteristics." } },
        ...
        { type:"p", text:{ fr:"Algorithme en OCaml, tableau de listes d'adjacence pour le graphe, tas binaire pour la file de priorité de Dijkstra.", en:"Algorithm in OCaml, an array of adjacency lists for the graph, a binary heap for Dijkstra's priority queue." } },
        { type:"p", text:{ fr:"La récupération des données et le tracé sont en R (code en quasi-totalité repris).", en:"Data retrieval and plotting are in R (mostly reused code)." } },
        { type:"p", text:{ fr:"Réseau routier depuis OpenStreetMap, altitudes depuis la mission SRTM de la NASA.", en:"Road network from OpenStreetMap, elevation from NASA's SRTM mission." } },
        { type:"p", text:{ fr:"Implémentation validée sur un exemple synthétique.", en:"Implementation validated on a synthetic example." } },
        ...
        { type:"p", text:{ fr:"Une portion d'environ 60 km² de l'agglomération lyonnaise, un graphe de plus de 100 000 sommets et arêtes.", en:"A roughly 60 km² slice of the Lyon area, a graph of over 100,000 vertices and edges." } },
        { type:"p", text:{ fr:"Demande : 15 km et 500 m de dénivelé ; couronne d'exclusion de 3 à 6 km.", en:"Request: 15 km and 500 m of gain; exclusion crown of 3-6 km." } },
        { type:"gallery", items:[
        ...
        { type:"p", text:{ fr:"49 959 cycles simples générés ; la couronne a écarté environ 50 000 sommets.", en:"49,959 simple cycles generated; the crown removed about 50,000 vertices." } },
        { type:"p", text:{ fr:"Les écarts à la demande vont jusqu'à 8 km et 300 m.", en:"Gaps to the request reach up to 8 km and 300 m." } },
  ...
  proProjects: [
    { icon: "🎥", slug: "videostitcher",
      ...
      skills: { fr: ["Pipeline en deux phases : comme les caméras ne bougent jamais, la détection de points, l'appariement et l'estimation d'homographie (ORB + RANSAC) sont calculés une seule fois à la calibration ; seuls le warping, le placement de la couture et le blending sont refaits à chaque frame",
                ...
                en: ["Two-phase pipeline: because the cameras never move, feature detection, matching and homography estimation (ORB + RANSAC) run once at calibration; only warping, seam placement and blending recur per frame",
                     ...
                     "GPU implementation (PyTorch grid_sample / conv2d) inside a multi-threaded, multi-CUDA-stream pipeline with a transparent CPU fallback; it raised 1080p throughput from ~2.75 FPS (CPU) to ~32 FPS (GPU), roughly 11.6×",
    ...
    { icon: "🏥", slug: "arkhn",
      ...
      skills: { fr: ["Benchmark de plus de 20 modèles (OCR, embedding, LLM) et déploiement des meilleurs pendant la migration OpenAI (Azure) → modèles open source sur infrastructure souveraine : d'abord l'OCR, puis les LLM au cœur du produit",
                ...
                en: ["Benchmarked more than 20 models (OCR, embedding, LLM) and deployed the winners during the OpenAI (Azure) → open-source, sovereign-infrastructure migration: OCR first, then the LLMs at the core of the product",
                     "Served the open-source models on GPU with vLLM (Python), learning the deployment workflow from the infra team",
                     "Redesigned the four tools the chat agent can call (list patient-document names; open a document by name; retrieve chunks from a text query, via RAG; access compliance information) so the LLM opens the right document directly for simple lookups and only falls back to RAG when needed, cutting the text it reads and the cost per query (previously it ran RAG or opened several documents by default)",
